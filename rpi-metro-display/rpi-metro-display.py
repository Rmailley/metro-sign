...
def run_display(api_key, station_code_receiver, direction_receiver, font_file, lines_file_path, stations_file_path):
    global stations_file
    global lines_file

    # Set global file paths for this subprocess
    stations_file = Value(ctypes.c_wchar_p, stations_file_path)
    lines_file = Value(ctypes.c_wchar_p, lines_file_path)
...
def get_weather_data(station_id="KDCA"):
    ...
    try:
        ...
        if resp.status_code != 200:
            logging.error(f"Error getting weather data! Status: {resp.status_code}")
            return None
        ...
        tb = traceback.format_exc()
        logging.error("Error getting weather data:")
        logging.error(tb)
        return None
...
def main():
    ...
    run_displays = Process(target = run_display, args=(sys.argv[2],station_code_receiver,direction_receiver,sys.argv[5],sys.argv[6],sys.argv[7],))
    server.start()
    run_displays.start()
